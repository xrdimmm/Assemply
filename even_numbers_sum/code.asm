...
loop:
	...
	andi $t1, $t0, 1               # less significant bit check to determine number nature || If bit  == 0 then then $t1 == 0 (number is even). If bit is 1 then $t1 = 1 (number is odd)
	bne  $t1, $zero, loop          # if value in $t1 != 0 (number IS_NOT even) goto label loop else do nothing( proceed to next line)
